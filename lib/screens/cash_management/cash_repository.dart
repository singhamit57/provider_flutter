...
import 'package:koibanda_provider_flutter/screens/cash_management/model/cash_detail_model.dart';
...
Future<List<PaymentHistoryData>> getPaymentHistory(
  ...
  PaymentHistoryModel res = PaymentHistoryModel.fromJson(await handleResponse(
      await buildHttpResponse('payment-history?$bId',
  ...
  try {
    ...
    res = CashHistoryModel.fromJson(
      await handleResponse(await buildHttpResponse(
          'cash-detail?$pages$pPage$tdate$fdate$sType',
          method: HttpMethodType.GET)),
    );
...
Future<UserBankDetails> getUserBankDetail({required int userId}) async {
  return UserBankDetails.fromJson(await handleResponse(await buildHttpResponse(
      'user-bank-detail?user_id=$userId',
      method: HttpMethodType.GET)));
}

Future<BaseResponseModel> transferCashAPI(
    {required Map<String, dynamic> req}) async {
  return BaseResponseModel.fromJson(await handleResponse(
      await buildHttpResponse('transfer-payment',
  ...
  await showConfirmDialogCustom(
    ...
    onAccept: (p0) async {
      Map<String, dynamic> req = {
        ...
        "datetime":
            formatDate(DateTime.now().toString(), format: DATE_FORMAT_7),
