...
abstract class TimeSlotStoreBase with Store {
  ...
  Future<void> timeSlotForProvider() async {
    ...
    isTimeSlotAvailable =
        list.where((element) => element.slot.validate().isNotEmpty).isNotEmpty;
  }

  @action
  Future<void> removeSlotData({required SlotData value}) async {
    serviceSlotData
        .removeWhere((element) => element.day == value.day.validate());
  }

  @action
  Future<void> setForAllServices(
      {required bool value, bool isInitializing = false}) async {
